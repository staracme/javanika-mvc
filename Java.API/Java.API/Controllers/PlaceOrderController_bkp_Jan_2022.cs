...
namespace Java.API.Controllers
{
    ...
    public class PlaceOrderController : ApiController
    {
        ...
        public FinalResponse PlaceOrder(PlaceOrderModel om)
        {
            JAVADBEntities db = new JAVADBEntities();

            string sessionID = om.sessionID;
            int eventID = om.eventID;
            ...
            order.Name = om.name;
            order.Email = om.email;
            order.Mobile = om.mobile;
            order.Amount = Convert.ToDecimal(om.amount);
            order.CreatedDate = DateTime.Now;
            order.OrderNo = serialNo;
            order.Status = "SUCCESS";
            order.PaypalOrderID = om.id;
            order.PaymentStatus = "COMPLETED";
            order.Intent = om.intent;
